...
namespace GameTimeNext.Core.Application.Metadata
{
    public sealed class CFMetadataClassGenerator
    {
        ...
        private static void GenerateTXBasicClass(string dataManagerPath, string suffix, string tableName, List<GeneratedField> fields)
        {
            ...
            code.AppendLine("MigrationFactory.ToCsv.ExportCsvFileFor(connection, obj, MigrationFactory.ImportType.DevSync);");
            code.EndBlock();
            code.AppendEmptyLine();
            ...
            code.AppendLine($"MigrationFactory.ToCsv.ExportCsvFileFor(connection, \"{tableName}\", MigrationFactory.ImportType.DevSync);");
            code.EndBlock();
            code.AppendEmptyLine();
