...
namespace GameTimeNext.Core.Application.DataManagers
{
    public class TXGROUPBasic
    {
        ...
        public virtual void Delete(int gRID)
        {
            SQLiteConnection connection = AppEnvironment.GetDataBaseManager().GetConnection();
            EnsureOpen(connection);
        ...
        private static T1GROUP Map(SQLiteDataReader reader)
        {
            T1GROUP obj = new T1GROUP();
            obj.GRID = reader.IsDBNull(0) ? 0 : reader.GetInt32(0);
            obj.GRNA = reader.IsDBNull(1) ? string.Empty : reader.GetString(1);
            obj.GTYP = reader.IsDBNull(2) ? string.Empty : reader.GetString(2);
            obj.CRAT = reader.IsDBNull(3) ? string.Empty : reader.GetString(3);
            obj.CHAT = reader.IsDBNull(4) ? string.Empty : reader.GetString(4);
            obj.State = UIXTableObjectState.Available;
            return obj;
        }
